...
fn test_supported_memory_alloc_dealloc() {
  ...
  unsafe {
    ...
    assert_eq!(
      slice.len(),
      size,
      "Allocated size should match requested size"
    );
    ...
    assert_eq!(
      slice[size - 1],
      24,
      "Should be able to write to end of allocated memory"
    );
...
fn test_supported_memory_reserve_modify() {
  ...
  unsafe {
    ...
    assert_eq!(
      slice[0], 42,
      "Should be able to write after modifying protection"
    );
...
fn test_supported_invalid_size_alignment() {
  unsafe {
    ...
    if let Err(error) = result {
      assert!(
        matches!(error, SysError::InvalidArgument),
...
fn test_unsupported_system_alloc() {
  ...
  unsafe {
    ...
    if let Err(error) = result {
      assert!(
        matches!(error, SysError::Unsupported),
